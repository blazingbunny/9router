...
- **Public `/v1/models` exposes almost nothing** — just `id`, `object`, `owned_by`. No context window, reasoning, vision, anything. This is by design (same as OpenAI's own `/v1/models`), not a bug. Don't build tooling that assumes the public endpoint carries capability data.
