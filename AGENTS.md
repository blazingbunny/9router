...
- **Public `/v1/models` exposes almost nothing** — for static-catalog models, just `id`, `object`, `owned_by`. No context window, reasoning, vision, anything. This is by design (same as OpenAI's own `/v1/models`), not a bug. Live metadata can add `capabilities` only when a resolver returns `m.capabilities` directly (currently Kiro and Kimchi). `capabilitiesFromServiceKind` in `src/app/api/v1/models/route.js` is a separate fallback for `customKind || liveKind`; Qoder, Copilot, and Clinepass generally return only `id`/`name`, so don't build tooling that assumes capabilities for ordinary static or live-listed models.
