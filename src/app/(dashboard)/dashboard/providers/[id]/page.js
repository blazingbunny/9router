...
export default function ProviderDetailPage() {
	...
	const [fetchModelsMessage, setFetchModelsMessage] = useState(null);
	...
	// Fetch /v1/models from compatible provider and import into custom models
	const handleFetchModels = async () => {
		...
		if (!activeConnection) {
			setFetchModelsMessage("No active connection");
			return;
		}
		...
		try {
			...
			const data = await res.json();
			if (!res.ok) {
				setFetchModelsMessage(data.error || "Failed to fetch models");
				return;
			}
			const models = data.models || [];
			if (models.length === 0) {
				setFetchModelsMessage("No models returned");
				return;
			}
			let importedCount = 0;
			for (const model of models) {
				...
				const alreadyExists = customModels.some(
					(entry) =>
						...
						(entry.modelId || entry.id) === modelId,
				);
			...
			setFetchModelsMessage("Failed to fetch models");
		} finally {
		...
		<div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
			...
			{isCompatible && providerNode && (
				<Card>
					...
					{fetchModelsMessage && (
						<p
							className={`text-xs mt-2 ${fetchModelsMessage.startsWith("Imported") ? "text-green-500" : "text-red-500"}`}
						>
							{fetchModelsMessage}
						</p>
					)}
