...
export async function GET(request, { params }) {
	try {
		...
		if (isOpenAICompatibleProvider(connection.provider)) {
			...
			// Allow explicit override via providerSpecificData.modelsPath
			const modelsPath = connection.providerSpecificData?.modelsPath;
			let url;
			if (modelsPath && typeof modelsPath === "string") {
				url = `${baseUrl.replace(/\/$/, "")}${modelsPath.startsWith("/") ? modelsPath : "/" + modelsPath}`;
			} else {
				...
				if (trimmed.endsWith("/v1")) {
					...
					// Try /v1/models first; fall back to /models on 404
			...
			// Auto-fallback: if /v1/models 404'd, try /models
			if (
				!response.ok &&
				response.status === 404 &&
				!modelsPath &&
				...
				const fallbackUrl = url.replace(/\/v1\/models/, "/models");
		...
		if (isAnthropicCompatibleProvider(connection.provider)) {
			...
			let url;
			if (modelsPath && typeof modelsPath === "string") {
				url = `${baseUrl}${modelsPath.startsWith("/") ? modelsPath : "/" + modelsPath}`;
			} else {
				// Try /v1/models first; fall back to /models on 404
